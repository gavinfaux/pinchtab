...
Pinchtab is an HTTP server that wraps Chrome DevTools Protocol (CDP) to give AI agents browser control via a simple REST API.
...
- **Bridge** — the single-instance `pinchtab bridge` runtime used for one managed browser
